...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(LAB2_DigitalInput_Output_M) == (NULL));
  while (runModel) {
    ...
    LAB2_DigitalInput_Output_step();

    /* Get model outputs here */

    /* Trigger External Mode event */
    extmodeEvent(0, currentTime);
    stopRequested = !((rtmGetErrorStatus(LAB2_DigitalInput_Output_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(LAB2_DigitalInput_Output_M, "stopping the model");
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  LAB2_DigitalInput_Output_terminate();
  extmodeReset();
...
int app_main(int argc, char **argv)
{
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(LAB2_DigitalInput_Output_M, 0);
  ...
  /* Initialize model */
  LAB2_DigitalInput_Output_initialize();

  /* External Mode initialization */
  errorCode = extmodeInit(LAB2_DigitalInput_Output_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(LAB2_DigitalInput_Output_M, true);
    }
  }
