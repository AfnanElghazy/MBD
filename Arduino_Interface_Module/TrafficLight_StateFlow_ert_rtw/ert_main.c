...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(TrafficLight_StateFlow_M) == (NULL));
  while (runModel) {
    ...
    TrafficLight_StateFlow_step();

    /* Get model outputs here */

    /* Trigger External Mode event */
    extmodeEvent(0, currentTime);
    stopRequested = !((rtmGetErrorStatus(TrafficLight_StateFlow_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(TrafficLight_StateFlow_M, "stopping the model");
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  TrafficLight_StateFlow_terminate();
  extmodeReset();
...
int app_main(int argc, char **argv)
{
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(TrafficLight_StateFlow_M, 0);
  ...
  /* Initialize model */
  TrafficLight_StateFlow_initialize();

  /* External Mode initialization */
  errorCode = extmodeInit(TrafficLight_StateFlow_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(TrafficLight_StateFlow_M, true);
    }
  }
