...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(TrafficLight_M) == (NULL));
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(TrafficLight_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(TrafficLight_M, "stopping the model");
}
...
int app_main(int argc, char **argv)
{
  ...
  errorCode = extmodeInit(TrafficLight_M->extModeInfo, &rtmGetTFinal
    (TrafficLight_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(TrafficLight_M, true);
    }
  }
